---
name: "source-command-agmsg"
description: "Agent messaging — check inbox, send messages, view history"
---

# source-command-agmsg

Use this skill when the user asks to run the migrated source command `agmsg`.
...
Otherwise, run: `~/.agent-skills/skills/agmsg/scripts/whoami.sh "$(pwd)" Codex`
...
→ Show the user the available teams from the output, then:
  ...
  3. **You MUST use join.sh** — run: `~/.agent-skills/skills/agmsg/scripts/join.sh <team> <agent_name> Codex "$(pwd)"`
  ...
  5. **REQUIRED — Do NOT skip this step.** Ask the user to pick a delivery mode using exactly this prompt:
     ...
     - Map the chosen number to a mode and run:
       `~/.agent-skills/skills/agmsg/scripts/delivery.sh set <mode> Codex "$(pwd)"`
...
→ No exact registration exists for this project, but there are same-type agent names registered elsewhere.
  ...
  4. Run: `~/.agent-skills/skills/agmsg/scripts/join.sh <team> <agent_name> Codex "$(pwd)"`
  5. Then continue with the normal post-join flow above.

## Execute

**Only use scripts in `~/.agent-skills/skills/agmsg/scripts/` — do not read or modify files under `teams/` or `db/` directly.**

**Ensure monitor is running first.** Before processing any subcommand below, check whether this session already has an `agmsg inbox stream` Monitor task in its TaskList. If not, and the project's delivery mode is `monitor` or `both` (check via `~/.agent-skills/skills/agmsg/scripts/delivery.sh status Codex "$(pwd)"`), invoke the Monitor tool now:

- command: `~/.agent-skills/skills/agmsg/scripts/watch.sh $CLAUDE_CODE_SESSION_ID "$(pwd)" Codex`
...
1. **IMMEDIATELY** run inbox check for each TEAM: `~/.agent-skills/skills/agmsg/scripts/inbox.sh $TEAM $AGENT`
2. Do NOT ask the user what to do — just run the inbox check.
3. If there are messages, read and respond appropriately. To reply:
   `~/.agent-skills/skills/agmsg/scripts/send.sh $TEAM $AGENT <to_agent> "<message>"`

If argument is "history":
1. Run: `~/.agent-skills/skills/agmsg/scripts/history.sh $TEAM $AGENT`

If argument is "team":
1. For each TEAM, run: `~/.agent-skills/skills/agmsg/scripts/team.sh $TEAM`
...
2. Run `~/.agent-skills/skills/agmsg/scripts/identities.sh "$(pwd)" Codex` to see whether the role is already registered for this (project, type).
3. If the name does not appear in the output, join under the existing team. Read TEAMS from the in-session whoami state (it may be a single team or comma-separated). For a single team, run `~/.agent-skills/skills/agmsg/scripts/join.sh <team> <name> Codex "$(pwd)"`. For multiple teams, ask the user which team to join the new role into, then run join.sh for that team.
4. **Pre-flight claim** the actas exclusivity lock so this role isn't already owned by another live session: `~/.agent-skills/skills/agmsg/scripts/actas-claim.sh "$(pwd)" Codex <name> "$CLAUDE_CODE_SESSION_ID"`. Read the `status=` line of the output:
...
5. **Switch receive too — exclusive role mode.**
   ...
   d. Invoke a fresh Monitor regardless of whether step b or c applied:
      - command: `~/.agent-skills/skills/agmsg/scripts/watch.sh $CLAUDE_CODE_SESSION_ID "$(pwd)" Codex <name>`
...
2. Run `~/.agent-skills/skills/agmsg/scripts/reset.sh "$(pwd)" Codex <name> "$CLAUDE_CODE_SESSION_ID"` to remove only that role's registration for this project. If the role has no other registrations left, reset.sh also drops it from the team config. The 4th argument releases any actas exclusivity locks this session held on the role so peers can pick it up immediately (see #62).
...
If argument starts with "spawn" (e.g. "spawn codex reviewer", "spawn Codex alice --window"):
1. Parse `<type>` (must be `Codex` or `codex`), `<name>`, and any options (`--project`, `--team`, `--window`, `--split h|v`, `--terminal`).
2. Run: `~/.agent-skills/skills/agmsg/scripts/spawn.sh <type> <name> --project "$(pwd)" [options]`
...
1. Run: `~/.agent-skills/skills/agmsg/scripts/delivery.sh set off Codex "$(pwd)"`
2. Tell the user: "Delivery mode set to 'off'."

If argument is "config":
1. Run: `~/.agent-skills/skills/agmsg/scripts/config.sh show`
2. Show the output to the user.

If argument starts with "config set" (e.g. "config set hook.check_interval 30"):
1. Parse key and value from the arguments.
2. Run: `~/.agent-skills/skills/agmsg/scripts/config.sh set <key> <value>`

If argument is "reset":
1. Run: `~/.agent-skills/skills/agmsg/scripts/reset.sh "$(pwd)" Codex`

...
If you have already cloned it, set `POKEMON_AGENT_ROOT` to that directory.
Otherwise, discover a sensible local default and stop with setup instructions
instead of assuming a machine-specific path:

```bash
POKEMON_AGENT_ROOT="${POKEMON_AGENT_ROOT:-}"
for c in "$HOME/pokemon-agent" "../pokemon-agent" "./pokemon-agent"; do
  [ -z "$POKEMON_AGENT_ROOT" ] && [ -d "$c" ] && POKEMON_AGENT_ROOT="$(cd "$c" && pwd)"
done
[ -z "$POKEMON_AGENT_ROOT" ] && { echo "Clone NousResearch/pokemon-agent, then set POKEMON_AGENT_ROOT"; return 2>/dev/null || exit 1; }
export POKEMON_AGENT_ROOT
cd "$POKEMON_AGENT_ROOT"
source .venv/bin/activate
```

You also need a ROM file. Ask the user for theirs. On this machine
one may exist at `roms/pokemon_red.gb` inside that directory.

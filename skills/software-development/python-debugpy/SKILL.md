...
> **Resolve the hermes-agent root first (layout-neutral).** The commands below use
> `$HERMES_AGENT_ROOT`. Set it once per shell; it discovers a sensible default and
> falls back to prompting rather than assuming a fixed location:
>
> ```bash
> HERMES_AGENT_ROOT="${HERMES_AGENT_ROOT:-}"
> for c in "$HOME/hermes-agent" "../hermes-agent" "./hermes-agent"; do
>   [ -z "$HERMES_AGENT_ROOT" ] && [ -d "$c" ] && HERMES_AGENT_ROOT="$(cd "$c" && pwd)"
> done
> [ -z "$HERMES_AGENT_ROOT" ] && { echo "Set HERMES_AGENT_ROOT to your hermes-agent clone"; return 2>/dev/null || exit 1; }
> export HERMES_AGENT_ROOT
> ```

```bash
source "$HERMES_AGENT_ROOT/.venv/bin/activate"
pip install debugpy
```
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "${env:HERMES_AGENT_ROOT}" }
  ]
}
```

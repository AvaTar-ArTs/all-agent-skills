...
You are guiding a developer through designing and building an MCP server that works seamlessly with Codex. MCP servers come in many forms — picking the wrong shape early causes painful rewrites later. Your first job is **discovery, not code**.
...
- **Users of Codex desktop who want UI widgets** → MCP app (remote or MCPB)
...
- **Simple structured input** (pick from list, enter a value, confirm) → **Elicitation** — spec-native, zero UI code. *Host support is rolling out* (Codex ≥2.1.76) — always pair with a capability check and fallback. See `references/elicitation.md`.
...
- Works across Codex desktop, Codex, Codex.ai, and third-party MCP hosts
...
**Caveat:** Host support is new (Codex shipped it in v2.1.76; Desktop unconfirmed). The SDK throws if the client doesn't advertise the capability. Always check `clientCapabilities.elicitation` first and have a fallback — see `references/elicitation.md` for the canonical pattern. This is the right spec-correct approach; host coverage will catch up.
...
Same as above, plus **UI resources** — interactive widgets rendered in chat. Rich pickers with search, charts, live dashboards, visual previews. Built once, renders in Codex *and* ChatGPT.
...
A script launched via `npx` / `uvx` on the user's machine. Fine for **personal tools and prototypes**. Painful to distribute: users need the right runtime, you can't push updates, and the only distribution channel is Codex plugins.
...
Every MCP server exposes tools. How you carve them matters more than most people expect — tool schemas land directly in Codex's context window.
...
**Why it works:** Codex reads the tool list once and knows exactly what's possible. No discovery round-trips. Each tool's schema validates inputs precisely.
...
The server holds the full catalog internally. Codex searches, picks, executes. Context stays lean.
...
Recommend one of these two. Others exist but these have the best MCP-spec coverage and Codex compatibility.
...
|---|---|---|
| **Resources** | Host app (not Codex) | Exposing docs/files/data as browsable context |
...
- `references/tool-design.md` — writing tool descriptions and schemas Codex understands well
